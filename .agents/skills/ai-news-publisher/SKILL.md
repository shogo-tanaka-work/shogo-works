...
- **Claude Code と Codex は通常のパッチを単独記事にしません。** 破壊的変更・権限・セキュリティ・料金・操作フローの変更に当たるものだけを単独記事にし、両ツール合わせて1回2本までとします。該当しないものは `見送り（週次ロールアップへ）` として日次サマリーに残し、週1回 `weekly-ai-news-rollup` がまとめます。条件は `references/selection-rubric.md` の「日次リリース型ツールの扱い」にあります。
