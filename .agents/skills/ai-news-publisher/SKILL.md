...
- **Claude Code と Codex は、バージョン番号が付いたリリースを内容にかかわらず単独記事にしません。** `見送り（週次ロールアップへ）` として日次サマリーに残し、週1回 `weekly-ai-news-rollup` がまとめます。権限・セキュリティ・資格情報の修正が含まれていても同じ扱いです（この2ツールはほぼ毎回含むため、中身で判断すると版数の羅列に戻ります）。例外はバージョンリリースではない独立発表と、退役日・発効日・料金改定日が明記された告知の2つだけで、その場合も両ツール合わせて1回2本までとします。条件は `references/selection-rubric.md` の「日次リリース型ツールの扱い」にあります。
