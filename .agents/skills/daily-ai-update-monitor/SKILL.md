...
- **Claude Code と Codex の通常パッチは、詳細ファイルを作ったうえで記事化しません。** 破壊的変更・権限・セキュリティ・料金・操作フローの変更に当たるものだけを記事化候補に上げ、それ以外は日次サマリーの「公開記事化結果」へ `見送り（週次ロールアップへ）` と残します。週1回 `weekly-ai-news-rollup` がまとめます。判定条件は `ai-news-publisher/references/selection-rubric.md` の「日次リリース型ツールの扱い」にあります。
