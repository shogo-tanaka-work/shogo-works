...
1. 直近24時間の対象期間を決めます。
   ...
   - **`00:00:00 GMT` 固定の時刻は date-only として扱い、窓の開始時刻と時刻で比較しません。** 実際の公開は同日の後刻で、日次実行（JST 9時台 = UTC 0時台）の窓開始より数分前に落ちるため、時刻比較では前日・当日のどちらの窓にも入らず構造的に取りこぼします。date-only エントリは前日・当日の日付を候補とし、前回までの日次サマリーと `official-updates/` に記録済みの URL を除いた差分を記録対象にします。

2. `references/source-catalog.md` の **`## 日次巡回ソース`** を確認します。
   - 公式ソースだけを使います。
   - **`## 週次確認ソース`（GitHub Copilot / Microsoft Copilot / n8n / Cursor / xAI / Grok / Manus / Dify）は日次では確認しません。** これらは `weekly-ai-news-rollup` Skill が週1回まとめて巡回します。日次では、他ソースから関連する言及を見つけたときだけ確認します。確認していないものを「更新なし」として記録しません。
...
- **OpenAI Status も確認します**。Codex Cloud / Code Review / APIなど対象期間内のincidentは `category: incident` として記録します。短時間incidentは原則AIニュース化せず、日次サマリーと詳細メモに留めます。
...
- **GitHub Copilot / Microsoft Copilot / n8n / Cursor / xAI / Grok / Manus / Dify は週次確認ソース**で、日次巡回では見ません。これらの巡回は `weekly-ai-news-rollup` Skill の担当です（出力先は `docs/research/weekly-ai-updates/YYYY-MM-DD.md`）。日次側から巡回しません。
...
- **Claude Code と Codex のバージョンリリースは、詳細ファイルを作ったうえで記事化しません。** バージョン番号が付いたリリースは内容にかかわらず、日次サマリーの「公開記事化結果」へ `見送り（週次ロールアップへ）` と残します。権限・セキュリティ・資格情報の修正が含まれていても同じです。記事化候補に上げてよいのは、バージョンリリースではない独立発表（公式ブログ・製品ページでの新機能発表）と、退役日・発効日・料金改定日が明記された告知だけです。判定条件は `ai-news-publisher/references/selection-rubric.md` の「日次リリース型ツールの扱い」にあります。
...
Cloudflare の詳細ファイルには必ず次を入れます。

- `product`（changelog の RSS タイトル先頭に出る製品名。例: `Workers AI`、`Agents, Workers`）
- `official_url`（changelog の該当エントリ、または blog の個別ポスト）

1つのエントリが複数製品にまたがる場合（`Agents, Workers - ...`）は、主たる製品を `service` に、残りを `product` に列挙します。changelog と blog の両方に同じ発表がある場合は、詳細が厚い blog 側を `source` にし、changelog URL を `## 原文確認` に併記します。

## n8nとGitHub Releases

**n8n は週次確認ソースです。日次巡回では確認しません。以下は `weekly-ai-news-rollup` から参照される規約です。**
