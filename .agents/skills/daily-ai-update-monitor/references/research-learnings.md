...
### 学び11: `00:00:00 GMT` 固定の date-only エントリは、時刻で窓判定すると構造的に漏れる

- **取りこぼし**: Cloudflare Browser Run「Session and DevTools methods」（`pubDate: 2026-09-21 00:00:00 GMT`、`developers.cloudflare.com/changelog/post/2026-09-21-browser-binding-methods/`）。2026-09-22 の巡回で漏れ、2026-09-23 の巡回で追補した。
- **原因**: date-only の `00:00:00 GMT` を実時刻として扱い、窓（`2026-09-21T00:02Z` 開始）と時刻で比較した。実際の公開は同日の後刻で、09-21 の巡回時点ではまだ出ておらず、09-22 の巡回では「窓開始の2分前」と判定された。**日次実行が UTC 0時台のため、窓を前回の window_end から連結しても date-only エントリは毎回この隙間に落ちうる。** SKILL.md の「日付しかないソースは今日または昨日の日付を対象」は、RSS に形式上の時刻が入っていると適用されなかった。
- **対処**: `00:00:00 GMT` 固定は date-only として日付で判定し、前日・当日の日付を候補にする。重複は記録済み URL との差分で除く。Cloudflare 横断フィードは約1週間分（20件前後）しか保持しないため、差分確認はフィード全体で行える。
- **カタログ取り込み**: 済（2026-09-23、SKILL.md 手順1と source-catalog.md の Cloudflare RSS 節へ反映）。
