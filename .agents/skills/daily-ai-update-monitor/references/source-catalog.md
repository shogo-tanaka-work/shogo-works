...
| OpenAI Codex | https://github.com/openai/codex/releases | https://learn.chatgpt.com/docs/changelog（`developers.openai.com/codex/changelog` から 308 リダイレクト）、https://learn.chatgpt.com/docs/hooks | ほぼ毎日（alpha含む） | docs/research/openai-codex/official-updates |
...
**日次巡回では確認しません。** これらは `weekly-ai-news-rollup` Skill が週1回（クラウド routine: 毎週月曜 9:30 JST）まとめて巡回し、`docs/research/weekly-ai-updates/YYYY-MM-DD.md` に記録します。日次巡回からは、他ソースで関連する言及を見つけたときだけ確認します。
