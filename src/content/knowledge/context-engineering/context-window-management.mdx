...
LLMのコンテキストウィンドウは、システム指示、ツール定義、会話履歴、外部データ、モデル出力で共有する有限の予算です。APIやモデルによって出力上限の扱いは異なるため、利用中の公式仕様を確認した上で余白を確保します。
...
コンテキスト上限、最大出力、長文入力の料金、キャッシュ条件は頻繁に変わります。本記事では製品横断の固定表を正本にせず、利用時に各社のモデル仕様と料金ページを確認します。

公称上限が大きくても、関連情報を無制限に入れてよいわけではありません。ChromaのContext Rot研究が示すように、性能低下は入力長だけでなく、情報の類似性、配置、タスク、モデルによって変わります。固定の安全率ではなく、自分の評価セットで有効容量を測ります。

## トークン予算の配分戦略

### 基本配分モデルの例

コンテキストウィンドウを4つのゾーンに分割して管理します。以下の比率は初期仮説であり、推奨固定値ではありません。
...
同じシステムプロンプトやツール定義を繰り返し送信する場合、対応APIのプロンプトキャッシュでレイテンシや入力コストを下げられることがあります。割引率、最小入力長、保持時間はモデルと提供元の最新仕様を確認します。
