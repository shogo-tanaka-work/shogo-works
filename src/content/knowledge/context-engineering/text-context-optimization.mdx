...
LLMのコンテキストウィンドウは有限です。関連性の低い長文より、必要な根拠と制約に絞った入力の方が、精度・コスト・レイテンシの面で有利になる場合があります。

削減率だけを目標にすると、必要な例外や根拠まで失います。圧縮前後でタスク成功率と情報保持率を測り、品質を維持できる範囲で削減します。
...
JSON、YAML、表、箇条書きのトークン効率は、内容とトークナイザーで変わります。形式変換だけで固定割合を見込まず、構造の明確さ、スキーマ検証、実測トークン数で選びます。
