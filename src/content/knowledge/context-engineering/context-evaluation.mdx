---
title: "コンテキスト品質の評価と計測：RAGAS・DeepEvalで精度を数値化する"
...
評価を後付けすると、検索・生成・データ更新のどこで品質が落ちたかを切り分けられません。最初から小さな評価セットと失敗例を用意し、検索結果と最終タスクの両方を継続的に測ります。

RAGASやDeepEvalの指標は便利ですが、フレームワーク固有のスコアだけを目標にしません。実際の利用者が必要な情報へ到達できたか、根拠と回答が一致したかを最終評価に含めます。
