...
固有名詞やIDの完全一致と意味検索の両方が必要なら、ベクトル検索とキーワード検索を組み合わせるハイブリッド方式が候補になります。デフォルトで複雑化せず、評価セットで単独方式との差を確認します。
...
リランキングで改善する場合がありますが、効果は候補取得の品質と質問分布に依存します。Recall、最終回答の正確さ、追加レイテンシ、費用を合わせて比較します。
...
- **GraphRAGを比較する**: 複数エンティティの関係や経路を問う質問では候補になる。ベクトル検索、SQL、全文検索と同じ質問セットで比較する

## まとめ

ベクトル検索はRAGの有力な構成要素ですが、すべての検索に必要ではありません。埋め込みモデル、全文検索、チャンキング、メタデータ、リランキングを個別に比較し、最小構成でタスク成功率を満たす組み合わせを選びます。
