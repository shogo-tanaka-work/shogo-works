...
マルチエージェントシステムでは、市場分析役は市場データ、技術分析役は技術資料だけを見るなど、責務ごとにコンテキストを分離できます。専門性、並列性、秘密情報の境界が必要な場合に有効です。

一方、調整コスト、重複作業、統合時の情報損失も増えます。単一エージェントとの比較評価を行い、分割理由が明確な場合だけ導入します。

## コンテキスト分離の3パターン

### パターン1: Handoff（引き継ぎ）

エージェント間で**制御を明示的に移譲**するパターンです。OpenAI Agents SDKではHandoffとして公式に提供されています。
