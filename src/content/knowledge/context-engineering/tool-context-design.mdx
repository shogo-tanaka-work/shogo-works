...
LLMがツール（外部API、データベース、ファイルシステムなど）を呼び出す際、**ツールの定義自体がコンテキストウィンドウを消費**します。定義数とスキーマが増えるほど、トークンだけでなく選択肢と攻撃面も増えます。
...
実用的なツール定義では、名前、説明、入力スキーマを明確にします。正確な必須項目は、利用中のFunction Calling APIまたはMCP仕様で確認します。
...
適切なツール数は、モデル、説明の類似度、タスク、動的検索の有無で変わります。候補を増やしたときの選択精度、不要な呼び出し、コンテキスト消費を評価し、必要なら名前空間、動的ロード、ツール検索で候補を絞ります。
