...
プロジェクトルートに `CLAUDE.md` を置いて、GAS 特有の制約を明文化します。これは [ハーネスエンジニアリング](/knowledge/harness-engineering/harness-engineering-complete-guide/) の基本動作と同じで、AI に毎回同じ注意書きを渡す手間を省く仕組みです。
