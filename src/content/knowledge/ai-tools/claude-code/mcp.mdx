...
## 認証：`claude mcp login` / `logout`

認証が必要な MCP サーバーには、専用のサブコマンドでサインインできます。
...
以前は対話画面の `/mcp` から認証するのが主な導線でしたが、**ターミナルから直接ログイン・ログアウトできる**ようになりました。CI やセットアップスクリプトから扱いやすくなっています。

---

## 関連ドキュメント

MCP は本サイトの複数の記事で扱っている。目的に応じて読み分けてほしい。

| 知りたいこと | 読む記事 |
|---|---|
| **Claude Code で MCP を設定する手順**（この記事） | いまここ |
| MCP とは何かを、専門用語を使わずに | [9-1 Claude Code の MCP とは](/knowledge/ai-tools/claude-code-curriculum/09-1-what-is-mcp)（非エンジニア向け） |
| どのサーバーを入れるべきか、CLI で代替できないか | [MCPサーバー選定と最適化](/knowledge/harness-engineering/mcp-server-strategy) |
| Codex 側での接続方法 | [Codex CLI × MCP](/knowledge/ai-tools/codex/mcp-integration) |
| ツール定義がコンテキストを圧迫する問題 | [ツールコンテキスト設計](/knowledge/context-engineering/tool-context-design) |
