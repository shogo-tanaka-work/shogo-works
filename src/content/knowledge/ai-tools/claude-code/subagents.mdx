...
サブエージェントの動き方は、リリースを重ねるごとに変わっています。押さえておくべき変更点です。

| 変更 | 内容 |
|---|---|
| **バックグラウンド実行が既定に** | サブエージェントは既定でバックグラウンドで走ります。メインセッションを止めずに進みます |
| **fork mode が対話セッションで既定 ON** | Claude が `fork` 型のサブエージェントを要求でき、**会話全体とプロンプトキャッシュを引き継ぎます**。文脈を渡し直す必要がありません。`CLAUDE_CODE_FORK_SUBAGENT=0` で無効化できます |
| **1セッション 200 体の上限が撤廃** | 総数の制限はなくなりました。ただし同時実行数と入れ子の深さには上限が残ります |
| **入れ子が可能に** | バックグラウンドのチェーンは最大5階層まで |

fork 既定化の影響が大きい点に注意してください。**サブエージェントが会話履歴を持たない前提で書いた定義や運用手順は、見直す価値があります。**
...
| テスト生成・コメント追加 | 軽量〜標準モデル | 定型的な生成タスク |
...
**モデル名は世代交代が速い**
本記事に出てくるモデル名は執筆時点のものです。現在の既定と選択肢は `/model` で確認してください。コスト最適化の考え方（重いタスクほど高性能モデル、定型タスクほど軽量モデル）自体は世代が変わっても有効です。

---

## 関連ドキュメント

この記事は **Claude Code でのサブエージェントの設定と使い方**を扱う。
設計論は別記事にある。

- 分割の判断基準と失敗パターン: [サブエージェント設計パターン](/knowledge/harness-engineering/sub-agent-patterns)
- 複数エージェント間のコンテキスト共有: [マルチエージェントのコンテキスト設計](/knowledge/context-engineering/multi-agent-context)
- 経路制御とルーティング: [マルチエージェントルーティング](/knowledge/graph-engineering/multi-agent-routing)
- Codex 側の実装: [Codex のサブエージェント](/knowledge/ai-tools/codex/subagents)
