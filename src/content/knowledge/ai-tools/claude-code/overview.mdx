...
1. 関連ファイルを読み、現在の状態を把握する
2. 何をどの順で変えるか計画を立てる
3. 計画に沿ってファイルを編集・コマンドを実行する
4. ビルドやテストを走らせて結果を検証する
5. 失敗していれば原因を特定して再修正する
...
- **チームでスポット利用 / CI連携**：API。Headlessモード（`-p`）でCIから呼ぶ用途、Batch APIで並列処理する用途では従量のほうが扱いやすい
- **両方を組み合わせる**：日常の対話セッションはサブスク、CIや夜間バッチはAPIキー、という分離も実務では一般的

サブスクでも API でも、コスト最適化の打ち手は共通です。

- `/usage` でセッションの消費トークンと現在のレート制限位置を都度確認する
- タスクが切り替わったら `/clear`、続きの作業に入るなら `/compact` でコンテキストを掃除する
- ファイル検索や1行確認のような軽量タスクは `/model` で Haiku に切り替える
- サブエージェント（`--agents` / `.claude/agents/`）の `model` を Haiku 固定にしておくと、調査系の並列処理を安く回せる
...
- **2026年Q3**: モデル世代が進み、既定モデルが入れ替わりました。あわせて **Auto mode が Pro / Max / Team の新規セッションで既定**になり、承認プロンプトの前提が変わっています。
- **2026年Q3**: **カスタムスラッシュコマンドが Skills へ統合**されました。`.claude/commands/` は動作しますが、新規作成は `.claude/skills/` が推奨です。
- **2026年Q3**: サブエージェントの **fork mode が対話セッションで既定 ON** になり、会話全体とプロンプトキャッシュを引き継ぐようになりました。1セッションあたり 200 体の上限も撤廃されています。
- **2026年Q3**: **self-hosted environments**（Team / Enterprise）、**GitLab 対応**、**クロスセッションメッセージング**が追加されました。

> **モデル名と価格は本文に固定していません。**世代交代が速いため、現在の既定は `/model`、価格は公式の料金ページで確認してください。

## はじめて Claude Code に触れる方へ

この記事は全体像を一気に把握するための解説です。**専門用語を使わずに、順番に手を動かしながら学びたい場合**は、入門コースから始めてください。

- [1-1 Claude Code とは](/knowledge/ai-tools/claude-code-curriculum/01-1-what-is-claude-code) — 何ができるかを平易に
- [1-2 職種別ユースケース集](/knowledge/ai-tools/claude-code-curriculum/01-2-use-cases-by-role) — 自分の仕事のどこで使えるか
- [1-3 料金プラン比較](/knowledge/ai-tools/claude-code-curriculum/01-3-pricing-plans) — サブスクと API の選び分け
