...
AIエージェントは、明示的な完了条件がなければ実装を先に進め、テストを十分に追加しない場合があります。

TDDは、期待する振る舞いを実装前に観測可能にする有力な方法です。ただし曖昧なUX、探索的な試作、既存挙動の調査では、受入条件や特性テストを先に置く方が適する場合もあります。
...
重要な仕様では、テスト作成と実装の入力を分けたり、人間が受入条件を確認したりすると追認を減らせます。サブエージェント分離は選択肢の1つであり、必須ではありません。
