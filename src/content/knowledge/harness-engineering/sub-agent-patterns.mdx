...
この構造の利点はコンテキスト境界を作れることです。ただし共有ワークスペースや共通状態を介した影響は残るため、「完全なファイアウォール」とは見なしません。

## コンテキストファイアウォールの効果

### コンテキストを責務ごとに絞る

モノレポ全体を1つのコンテキストへ入れる代わりに、バックエンド、フロントエンド、調査、評価など、役割に必要な情報だけを渡せます。ただし親子間の依頼と結果統合にもコストがかかるため、単一エージェントとの比較を実測します。
...
自動生成した指示を無検証で蓄積すると、重複や矛盾によって推論コストと失敗が増えることがあります。エージェントが提案した知見も、人間が再現性と適用範囲を確認してから正本へ反映します。
...
この記事は**設計パターン**に絞っている。Claude Code での具体的な設定方法は
[サブエージェント活用ガイド](/knowledge/ai-tools/claude-code/subagents) を参照。
