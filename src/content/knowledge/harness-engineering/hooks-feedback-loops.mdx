...
Claude Code Hooksは、エージェントのライフサイクルの特定ポイントでコマンドや評価処理を起動する仕組みです。LLMへの依頼より決定論的にできますが、Hook自体の設定漏れ、タイムアウト、スクリプト失敗も監視します。
...
利用できるライフサイクルイベントはClaude Codeの更新で変わります。以下は代表例であり、設定前に公式Hooksリファレンスで現在のイベント名と入出力を確認します。
...
Claude Code の Hooks の設定方法とイベント一覧は
[Claude Code Hooks ガイド](/knowledge/ai-tools/claude-code/hooks) を参照。
