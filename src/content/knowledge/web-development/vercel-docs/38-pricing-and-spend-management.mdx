...
- Hobby プランには Active CPU 4 時間・Provisioned Memory 360 GB-時・Invocations 100 万回・Fast Data Transfer 100 GB・CDN Requests 100 万回・Build Execution 6,000 分などが含まれる。Pro はクレジットの範囲内で使い、超過分はリージョン別または公開単価で従量課金される。
- Functions（Active CPU $0.128/時、Provisioned Memory $0.0106/GB-時、Invocations $0.60/100 万回）、CDN Requests（リージョン別単価。Pro は 1,000 万回付与）、Image Optimization（$0.05–$0.0812/1K 変換）、Microfrontends（$2/100 万 Routing、追加プロジェクト $250/月）、Workflows（$20/100 万 Events、データ書込 $0.50/GB）、Observability Plus（Pro $10/月＋ $1.20/100 万 Events）といった単価が、機能ごとに公式に明記されている。
...
| CDN Requests 含み | 100 万回 | 1,000 万回 | カスタム |
...
| CDN Requests（請求画面の表示は Edge Requests） | 件 | リージョン別 | 1,000 万回 |
...
| Global Config Reads | 件 | $3.00/100 万 | 100 万 |
| Global Config Writes | 件 | $5.00/1,000（Pro 換算 $1.00/1,000 含み枠超過分） | 1,000 |
...
| Speed Insights Events | 件 | **$0.65/10,000 events** | 10,000 events |
...
> **Speed Insights の単位に注意してください。**単価は「データポイント1件あたり $0.65」ではなく **「10,000 events あたり $0.65」** です。1件あたりと読むと見積もりが 10,000 倍ずれます。
>
> さらに 2026-08-25 に **全プラン・プロジェクト数無制限の無料 tier** が新設されました。従来「Hobby は 1 プロジェクトのみ無料、Pro は add-on 必須」だった建付けは解消されています。
>
> ただし **changelog と料金ページの記述が一致していません。**changelog は有料版を **Speed Insights Plus** と呼びますが、料金ページには旧体系の「$10/月/プロジェクト」が残っており、$0.65 / 10,000 events がどちらの体系の単価なのかを一次資料で確定できませんでした。**見積もりに使う前に契約画面で確認してください。**
>
> 出典: [Limits and Pricing for Speed Insights](https://vercel.com/docs/speed-insights/limits-and-pricing) / [Speed Insights now has a free tier](https://vercel.com/changelog/speed-insights-free-tier)
...
DX Platform 側の追加機能は固定月額が中心である。Pro プランの代表的な add-on は SAML Single Sign-On（$300/月）、HIPAA BAA（$350/月）、Static IPs（$100/プロジェクト/月）、Preview Deployment Suffix（$100/月）、Flags Explorer（$250/月）、Speed Insights Plus（$10/プロジェクト/月）である（出典: [Pricing on Vercel](https://vercel.com/docs/pricing)）。これらは Spend Cap の対象外で月次固定で課金される。
...
| エッジ関数 | Vercel Functions の Edge ランタイム（V8 Isolate） | Lambda@Edge / CloudFront Functions | Workers（V8 Isolate） |
