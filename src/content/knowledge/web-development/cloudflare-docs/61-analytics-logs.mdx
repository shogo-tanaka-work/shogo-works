...
tags: ["Cloudflare", "Analytics", "ドキュメント要約"]
...
- **対応データセット**（主なもの）：HTTP requests、Firewall events / WAF、Spectrum events、DNS firewall、Gateway DNS / HTTP / Network、Access requests、Audit logs、Workers Trace events、Workers Invocations、R2 access、Magic IDS / Network Flow、CASB findings、Zaraz、DNS logs、Sinkhole、Device Posture results、Page Shield。
...
**Azure Functions ベースの Microsoft Sentinel コネクタが非推奨**になりました（2026-08-26 告知）。**2026-09-14 までに移行**が必要です。Sentinel へログを送っている場合は確認してください。

そのほかの変更です。

| 時期 | 変更 |
|---|---|
| 2026-08-26 | Log Explorer にデータセット削除機能 |
| 2026-08-21 | Web Analytics が SPA の soft navigation を正しく計測 |
