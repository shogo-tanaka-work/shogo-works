...
tags: ["Cloudflare", "Gateway", "ドキュメント要約"]
...
### Network ポリシー（L3-L4）
HTTPに収まらないトラフィック（SSH / RDP / SMTP / 任意のTCP・UDP）を制御する層である。Cloudflare WAN / GRE / IPSec / WARPで集約したパケットをIP・ポート・プロトコル・SNI単位で評価する。
...
| L4 / Network | 統合（Cloudflare WAN連携） | 専用ZPA / Cloud Connector | NewEdgeで対応 | SIG / Tunnel |
...
- **Enterprise**：年間契約。Logpush、Advanced DLP、CASB、SCIM強制再評価、User Risk Score、Cloudflare WAN統合、専用PoP・専用IP、サポートSLAなど。SaaS全社展開や監査要件のある組織はここを前提に設計する。
