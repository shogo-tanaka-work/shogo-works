...
tags: ["Cloudflare", "Bot Management", "ドキュメント要約"]
...
## AI トラフィックの制御（2026年の変更）

ここは 2026 年でもっとも動いた領域です。

### 3つの用途で個別に制御できる（2026-07-01、Free 含む全プラン）

AI クローラーを一括で許可／拒否するのではなく、**用途ごとに分けて判断**できます。

| 用途 | 何をするか |
|---|---|
| **Search** | 検索結果に載せるためのクロール |
| **Agent** | AI エージェントがユーザーの依頼で取りに来るアクセス |
| **Training** | モデルの学習データとして収集 |

**2026-09-15 以降に登録される新規ドメインは、広告収益ページで Training と Agent をブロックし、Search は許可する設定が既定**になります。既存ドメインの設定は変わりません。

「AI に読ませたくない」と「検索に出したい」は両立するので、この分割は実務的に効きます。

### Bot Preference Sync（2026-08-21、全プラン）

Cloudflare 側で設定した AI ボットのポリシーから、**`robots.txt` を自動生成して同期**します。新規は既定で有効です。

これまでは「Cloudflare のダッシュボードで拒否したのに `robots.txt` は許可のまま」という食い違いが起きがちでしたが、それが解消されます。

### そのほか

- **BotBase**（追跡しているボットの一覧）
- **Attribution Business Insights**（Enterprise Bot Management）
