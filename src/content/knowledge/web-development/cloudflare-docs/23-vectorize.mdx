...
tags: ["Cloudflare", "Vectorize", "ドキュメント要約"]
...
| 最大件数/Index | 2,000万 | 数億〜 | DBサイズ依存 | ノード次第 |
...
- **インデックスあたり最大2,000万ベクトル**（2026-08-04 に 1,000万から倍増）。これを超える規模は別インデックス分割（テナント別 / 期間別）の設計が必要。
