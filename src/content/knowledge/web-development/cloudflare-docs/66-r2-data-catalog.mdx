...
tags: ["Cloudflare", "R2 Data Catalog", "ドキュメント要約"]
...
- **R2 Data Catalog**：**2026-05-28 に料金が確定し、2026-08-03 から非 Enterprise アカウントで課金が始まっている**（$9.00 / 100万オペレーション、compaction は $0.005/GB 処理。無料枠あり）。プロダクト自体は引き続き public beta。
...
**「ベータ中は R2 の標準コストだけで回る」という前提は Data Catalog については既に成立しない。**採用前に、オペレーション数と compaction 量の見積もりが必要になる。R2 SQL 側はまだ無料。
