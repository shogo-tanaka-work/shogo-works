---
title: "Cloudflare Turnstile：reCAPTCHA不要でBot対策を実装する方法"
...
tags: ["Cloudflare", "Turnstile", "ドキュメント要約"]
...
## Turnstile Spin（2026-08-10 GA）

ウィジェットの作成から検証の配線までを一括で行える仕組みが GA しました。3つの経路があります。

| 経路 | 使いどころ |
|---|---|
| ダッシュボード | 画面から作る |
| Wrangler CLI | 手元から作る |
| **AI コーディングエージェント** | Claude Code などから「Turnstile を入れて」と頼む |

サーバー側の `siteverify` 呼び出しまで配線し、検証テストも内蔵されています。**「入れたつもりで検証していなかった」という典型的な事故を防げます。**
