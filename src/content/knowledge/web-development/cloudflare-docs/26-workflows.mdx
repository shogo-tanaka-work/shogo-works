---
title: "Cloudflare Workflows：長時間・多ステップ処理を確実に完遂する耐久実行エンジン"
...
tags: ["Cloudflare", "Workflows", "ドキュメント要約"]
...
> **2026-08-10 以降、step 単位の課金が始まっている。**それ以前は「Workers Standard と同一で追加料金なし」だったが、CPU・リクエスト・ストレージに加えて **step が第4の課金軸**になった。

- **step 実行数**（2026-07-07 告知、2026-08-10 課金開始）: Workers Paid は月50万 step まで無料、超過分は10万 step あたり $0.80。Free プランは1日3,000 step。
...
- **コスト最適化観点**: 長時間スリープ自体は課金されないため、リマインダーやスケジュールジョブを走らせる用途には引き続き強い。ただし **step 数が課金軸に加わったため、1ワークフローを細かく刻みすぎると効いてくる**。ステップ分割は「失敗したときにどこから再開したいか」を基準に決める。
...
| 時期 | 変更 |
|---|---|
| 2026-06-02 | **binding に cron スケジュールを直接付与**できる |
| 2026-06-05 | **saga 形式の rollback** — 失敗時に逆順で補償処理を走らせる |
| 2026-06-23 | rollback ハンドラへ step context を渡せる |
| 2026-07-09 | `retries.delay` に関数を渡して**動的リトライ遅延** |
| 2026-08-04 | **CI/CD Workflows** — Artifacts リポジトリの自動ビルド・デプロイ |

**saga rollback** は、途中で失敗したときに「それまでにやったことを取り消す」処理を宣言的に書ける仕組みです。決済や在庫引き当てのように、中途半端な状態で止まると困る処理で効きます。
