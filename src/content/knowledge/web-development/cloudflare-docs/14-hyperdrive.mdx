---
title: "Cloudflare Hyperdrive：既存DBへの接続を高速化するプーリング＋キャッシュ"
...
tags: ["Cloudflare", "Hyperdrive", "ドキュメント要約"]
...
- **MySQL系**: MySQL本体、PlanetScale、AWS RDS MySQL、Aurora MySQL、Google Cloud SQL MySQL等。**MySQL サポートは 2026-08-07 に GA**（それ以前は GA ではなかった）。あわせて接続プールの手動リスタートが可能になった
- PlanetScale の Postgres / MySQL は、2026-06-18 以降 Hyperdrive のダッシュボードから直接作成できる
- プライベートネットワーク上のDBへは **Workers VPC 経由が公式の推奨経路**（2026-04-29）
