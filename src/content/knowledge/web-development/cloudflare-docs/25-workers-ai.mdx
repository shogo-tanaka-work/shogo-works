...
tags: ["Cloudflare", "Workers AI", "ドキュメント要約"]
...
## AI Gateway との関係：2026-08 に統合された

> **2026-08-07 の Agents Week で、Workers AI と AI Gateway は単一の AI コントロールプレーンへ統合されました。**「AI Gateway をフロントに置くプロキシ」という従来の理解は、現在の構成と合いません。

統合によって変わった点です。

| | 統合前 | 統合後 |
|---|---|---|
| 呼び出し | Workers AI と外部プロバイダで別経路 | **`env.AI.run()` が両方に対応** |
| gateway の作成 | 明示的に作る必要があった | **明示しなくても default gateway が自動生成される** |
| クレジット | 別建て | AI Gateway のプリペイドクレジットが **Workers AI の推論にも充当される** |
| レート上限 | — | 統合課金の利用でフロンティアモデルが緩和（50 req/min） |

引き続き使える機能です。

- キャッシュ（同一プロンプトの再利用でコスト削減）
- レート制限・リトライ・モデルフォールバック
- ログ・分析・コスト可視化の一元化
- **spend limit**（2026-06-05 追加）— 金額ベースの予算を設定し、超過時にブロックする
- **User Insights ダッシュボードと Access 連携の identity-aware 制御**（2026-08-05 追加）

典型構成: Workers → `env.AI.run()`（Workers AI / 外部プロバイダ）→ Vectorize で記憶。
