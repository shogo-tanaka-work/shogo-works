...
tags: ["Cloudflare", "Tunnel", "ドキュメント要約"]
...
これによりVPN置換・SaaS的アクセス体験・きめ細かな条件付きアクセス（デバイス姿勢、地理、時間帯）を実現できる。SSH/RDPもブラウザレンダリングで提供可能（**Browser Run** 連携。2026-04-15 に Browser Rendering から改称）。
...
## Cloudflare Mesh との使い分け

2026-04-14 に **Cloudflare Mesh** が登場し、WARP Connector が mesh node へ改称されました。Tunnel との役割が分かれます。

| | Cloudflare Tunnel | Cloudflare Mesh |
|---|---|---|
| 向き | **外向き**。内部サービスをインターネットへ公開する | **内部同士**。拠点・ユーザー・Workers をプライベート網でつなぐ |
| 到達方法 | 公開ホスト名（`app.example.com`） | プライベートIPでの相互到達 |
| 典型用途 | 社内Webアプリの公開、開発環境の共有 | 拠点間接続、エージェントへの限定的なアクセス付与 |

「外に出すなら Tunnel、内側でつなぐなら Mesh」と覚えると判断しやすくなります。
