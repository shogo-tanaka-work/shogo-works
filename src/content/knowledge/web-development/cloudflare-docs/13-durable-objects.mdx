...
tags: ["Cloudflare", "Durable Objects", "ドキュメント要約"]
...
## 設定方法の変更：`migrations` から `exports` へ

2026-07-04 に、Durable Object クラスを宣言する方法が変わりました。

| | 旧（`migrations`） | 新（`exports`） |
|---|---|---|
| 書き方 | `tag` を振って `new_sqlite_classes` を積み上げる | クラス名に対してストレージ種別を宣言する |
| 状態 | 既存プロジェクトは動作する | **新規はこちら** |

宣言的な `exports` フィールドが `migrations` を置き換えました。既存の設定はそのまま動きますが、**新しく書くなら `exports` を使ってください。**

あわせて **2026-07-09 以降、新規ネームスペースは SQLite バックエンドが必須**になりました。KV バックエンドのネームスペースは新規作成できません。
...
| 時期 | 変更 |
|---|---|
| 2026-06-19 | `apac-ne` / `apac-se` のロケーションヒント |
| 2026-06-26 | **`us` jurisdiction** — 米国内限定の配置 |
| 2026-06-30 | メモリ使用量チャート（P50〜P999） |
| 2026-08-25 | `ctx.abort()` 時に `retryAlarm: false` を指定できる |
