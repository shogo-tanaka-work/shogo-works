...
tags: ["Cloudflare", "AI Gateway", "ドキュメント要約"]
...
## 一行サマリ
複数LLMプロバイダに対する観測（ログ・分析・コスト追跡）、キャッシュ、レート制限、フォールバック、リトライ、ガードレールを提供するCloudflareのマネージドサービス。**2026-08-07 に Workers AI と統合され、単一の AI コントロールプレーンになった。**

> **「前段に置くプロキシ」という理解は現在の構成と合わない。**統合後は `env.AI.run()` が Workers AI と外部プロバイダの双方を扱い、明示的に gateway を作らなくても default gateway が自動生成される。AI Gateway のプリペイドクレジットは Workers AI の推論にも充当される。詳細は [Workers AI](./workers-ai) を参照。
...
| 時期 | 内容 |
|---|---|
| 2026-05-21 | **統一 REST API**。`api.cloudflare.com` 経由で Workers AI と外部プロバイダを同一 API から扱える |
| 2026-06-05 | **spend limit**。金額ベースの予算を設定し、超過時にブロックする |
| 2026-06-12 | ログに User-Agent を記録 |
| 2026-08-05 | **User Insights ダッシュボード**と、Access 連携の identity-aware 制御 |
| 2026-08-07 | **Workers AI との統合**（上記） |
