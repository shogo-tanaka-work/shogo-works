---
title: "Cloudflare Pages：Git連携からSSRまで動くエッジホスティング"
...
tags: ["Cloudflare", "Pages", "ドキュメント要約"]
...
## 新規プロジェクトは Workers が推奨

公式ドキュメントの誘導が強くなっています。原文です。
...
**既存の Pages プロジェクトは動き続けます**が、これから作るなら Workers（Static Assets）を選ぶのが公式の方針です。本記事は既存プロジェクトの運用と、Pages 固有の挙動を理解するための資料として読んでください。

なお 2026-08-11 から、キューイングされたビルドは自動でスキップされるようになりました。
