---
title: "Claude Code v2.1.157、`.claude/skills` プラグイン自動ロードと EnterWorktree セッション中断なし切替を投入 — `claude plugin init` でスキャフォールド、`/plugin` 引数補完、worktree 自動 unlock、`tool_decision` に bash / MCP / skill 名、ゼロバイト画像 crash 修正、auto モードのネットワーク権限プロンプト誤発生修正ほか多数。翌 v2.1.158 で Bedrock / Vertex / Foundry の Opus 4.7 / 4.8 で Auto mode が opt-in 解禁"
tool: "claude-code"
