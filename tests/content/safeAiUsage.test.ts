...
const SERIES_DIR = join(
  process.cwd(),
  "src/content/knowledge/ai-governance/safe-ai-usage",
);
const SUBCATEGORY = "safe-ai-usage";
/** index を除いた本編の記事数 */
const ARTICLE_COUNT = 6;

/**
 * 研修教材を出典とするシリーズのため、受講企業を特定できる語が本文へ紛れ込まないよう
 * 機械的に検査する。表記ゆれを含めて列挙する。
 */
...
describe("業務AI安全利用シリーズの整合性", () => {
  it("index と本編6本で構成されていること", () => {
  ...
  it("sortOrder が 1 から連続していること", () => {
    const orders = body.map((article) => article.sortOrder).sort((a, b) => a - b);
  ...
  it("受講企業を特定できる語を含まないこと", () => {
    const hits: string[] = [];
    articles.forEach((article) => {
      FORBIDDEN_TERMS.forEach((term) => {
        if (article.body.includes(term)) hits.push(`${article.file}: ${term}`);
      });
    });
    expect(hits, `固有名詞の混入:\n${hits.join("\n")}`).toEqual([]);
  });
