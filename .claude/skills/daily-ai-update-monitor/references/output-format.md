...
※ この節は「AIニュース記事化しないもの」だけに書きます。記事化するものは省略します（下の「教材化メモ節の扱い」参照）。
...
詳細ファイルの `## 教材化メモ` は、その更新をAIニュース記事化するかどうかで扱いを変えます。

- **記事化する**: 詳細ファイルの `## 教材化メモ` 節は**書きません**。教材化の観点は `src/content/ai-news-notes/<tool>/<slug>.mdx` に集約します。詳細ファイルには代わりに `教材化メモ: src/content/ai-news-notes/<tool>/<slug>.mdx` の1行だけを置きます。
- **記事化しない**（見送り・保留・tool enum 外）: 対になる教材化メモが作られないため、詳細ファイルに `## 教材化メモ` を書きます。

同じ内容を詳細ファイルと教材化メモの両方に書くと、更新時に片方だけ古くなります。記事化するものは教材化メモ側を正本とします。

## 本文量の目安

- 詳細ファイル1件あたり **1,000字以内**。`## 公式内容の日本語要約` は3〜5段落に収めます。
- 日次サマリーは **6,000字以内**。更新件数が多い日は「更新なし」節の各行を1文に切り詰めます。
- 原文の再構成が長くなりそうな場合は、記事側（`src/content/ai-news/`）に厚みを持たせ、詳細ファイルは索引として短く保ちます。
